...
class SessionUtil
{
    ...
    {
        self::loadConfig();
        return (self::$config['environment'] ?? 'production') === 'production';
    }
     ...
     * VERBESSERT: Verhindert doppelte Regeneration
     */
    private static function handleSessionSecurity(): void
    {
        // Check if session is expired
        if (self::isSessionExpired()) {
            LogUtil::logAction(LogType::SECURITY, 'SessionUtil', 'handleSessionSecurity', 'Session expired - destroying');
            self::destroy();
            return;
        }
        ...
        if (!self::validateSecurityFingerprint()) {
            LogUtil::logAction(LogType::SECURITY, 'SessionUtil', 'handleSessionSecurity', 'Fingerprint validation failed - destroying session');
            self::destroy();
            return;
        }

        // Regenerate session ID periodically (ABER NUR WENN NÖTIG)
        if (self::shouldRegenerateId()) {
            LogUtil::logAction(LogType::SECURITY, 'SessionUtil', 'handleSessionSecurity', 'Periodic regeneration needed');
            self::regenerateId();
        }
     ...
     * PRODUCTION-VERSION ohne Debug-Logging
     */
    private static function generateSecurityFingerprint(): string
    {
        ...
        // ENTWICKLUNGSUMGEBUNG: Nur in Development-Modus
        if (!self::isProduction()) {
            // Development: IP-basiert für DevTools-Kompatibilität
            return hash('sha256', $remoteAddr . '_dev_mode');
        }

        // PRODUCTION: Moderate Browser-Familie-Erkennung
        $isChromeFamily = strpos($userAgent, 'Chrome') !== false ||
            strpos($userAgent, 'Chromium') !== false ||
            strpos($userAgent, 'Edg/') !== false;

        if ($isChromeFamily) {
            // Chrome-Familie: Basis ohne Mobile/Desktop-Unterscheidung
            // Aber immer noch IP + Language für zusätzliche Sicherheit
            $browserCore = 'chrome_family';
            if (strpos($userAgent, 'Edg/') !== false) {
                $browserCore = 'edge_chromium';
            }

            // WICHTIG: RemoteAddr bleibt für Security
            return hash('sha256', $remoteAddr . $acceptLanguage . $browserCore);
        }

        // Andere Browser: Vollständiger Fingerprint
        return hash('sha256', $userAgent . $acceptLanguage . $remoteAddr);
    }

    /**
     * Validate security fingerprint - PRODUCTION VERSION
     */
    private static function validateSecurityFingerprint(): bool
    {
        if (!isset($_SESSION['security_fingerprint'])) {
            // Nur bei Development loggen
            if (!self::isProduction()) {
                LogUtil::logAction(LogType::SECURITY, 'SessionUtil', 'validateSecurityFingerprint', 'No stored fingerprint found');
            }
            return false;
        }

        $currentFingerprint = self::generateSecurityFingerprint();
        $storedFingerprint = $_SESSION['security_fingerprint'];
        $isValid = hash_equals($storedFingerprint, $currentFingerprint);

        // Nur Failures loggen in Production
        if (!$isValid) {
            LogUtil::logAction(LogType::SECURITY, 'SessionUtil', 'validateSecurityFingerprint', 'Fingerprint validation failed');
        }
    ...
    /**
     * Session für Login vorbereiten (verhindert doppelte Regeneration)
     */
    public static function prepareForLogin(): void
    {
        // Prüfen ob kürzlich regeneriert wurde (innerhalb der letzten 60 Sekunden)
        $lastRegenerated = $_SESSION['last_regenerated'] ?? 0;
        $timeSinceRegeneration = time() - $lastRegenerated;

        if ($timeSinceRegeneration < 60) {
            // Kürzlich regeneriert - nur Fingerprint und Activity updaten
            $_SESSION['security_fingerprint'] = self::generateSecurityFingerprint();
            $_SESSION['last_activity'] = time();
