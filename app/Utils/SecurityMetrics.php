...
class SecurityMetrics {
    ...
    private static function calculateSecurityScore($summary, $alerts): int {
        ...
        if ($summary['failed_logins'] > 50) $score -= 10;
        if ($summary['failed_logins'] > 100) $score -= 20;
        
        // Abzug für Alerts
        foreach ($alerts as $alert) {
            if ($alert['level'] === 'HIGH') $score -= 15;
            if ($alert['level'] === 'MEDIUM') $score -= 5;
        }
    ...
    public static function logDailySummary(): void {
        ...
        LogUtil::logAction(LogType::AUDIT, 'SecurityMetrics', 'dailySummary', 
        ...
        foreach ($alerts as $alert) {
            if ($alert['level'] === 'HIGH') {
                LogUtil::logAction(LogType::SECURITY, 'SecurityMetrics', 'criticalAlert', 
